...
async function creatConnection(connetInfo, action) {
    ...
    for (let i = 0; i < connetInfo.length; i++) {
        ...
        //action = "write";
        action == 'read' ? readData(connetInfo[i], client, session) : writeData(connetInfo[i], client, session);
    }
}
...
const dataInitial = (arr) => {
    let connetInfo = arr; //初始化所有的server連線資訊
    // console.log("connetInfo:");
    // console.log(connetInfo);

    //讀取所有server資料點資料
    creatConnection(connetInfo, "read");
